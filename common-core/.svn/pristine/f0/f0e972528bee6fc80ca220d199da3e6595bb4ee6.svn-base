...
function pushInGraphData(type_chart,data,config,pushInfoDefault) {
	...
	function retornarValorTotal(data){
		var total = 0;
		for(var posicion=0; posicion < data.length; posicion++){
			total = total + Number(data[posicion].value);
		}
		return total;
	}
	
	function calcularPorcentaje(valorTotal, valorUnitario){
		return (Number(valorUnitario)/Number(valorTotal))*100;
	}
	...
	function pushInGraphDataSub(type_chart,pushInfoI,i,j,shapesInChart,data,config,resconfig) {
		...
		if (newShapes.text.indexOf("<Value>") >= 0){
			...
			if(j==-1)newShapes.text=newShapes.text.replace("<Value>",""+formatoNumeroComas(pushInfoI.value));
			else newShapes.text=newShapes.text.replace("<Value>",""+formatoNumeroComas(pushInfoI.data[j]));
			/* DI - Fin */
		}
		var valorTotal = retornarValorTotal(data);
		console.log('valorTotal: ' + valorTotal);
		var porcentaje = calcularPorcentaje(valorTotal, pushInfoI.value);
		if (newShapes.text.indexOf("<Porcentaje>") >= 0) newShapes.text=newShapes.text.replace("<Porcentaje>",porcentaje.toFixed(2));
