...
function loadMultiSelectRepresa() {
	$('#cbxTipoManiobra').multiselect({
		disableIfEmpty: true,
		includeSelectAllOption: true,
        selectAllValue: 'select-all-value',
        nonSelectedText: 'SELECCIONE MANIOBRAS',
        nSelectedText: ' MANIOBRAS SELECCIONADOS',
...
llenarDatos = function () {
	...
	$.ajax({
 	   type: 'GET',
 	   url: "reporteCaudales/obtenerData",
	   ...
	   complete: function(result) {
		   if(len == 0){
				  ...
				  $.confirm({
					    ...
					    content: 'No hay datos registrados para el per&iacute;odo y maniobra(s) ingresados',
					    buttons: {
					    	Aceptar: { 
						        }
						    }
