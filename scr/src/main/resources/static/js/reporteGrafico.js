...
function loadMultiSelectRio() {
	$('#cbxRio').multiselect({
		disableIfEmpty: true,
		includeSelectAllOption: true,
        selectAllValue: 'select-all-value',
        nonSelectedText: 'SELECCIONE R\u00CDOS',
        nSelectedText: ' R\u00CDOS SELECCIONADOS',
...
llenarDatos = function () {
	...
	$.ajax({
 	   type: 'GET',
 	   url: "reportesGrafico/obtenerData",
	   ...
	   complete: function(result) {
		  if(len == 0){
			  ...
			  $.confirm({
				    ...
				    content: 'No hay datos registrados para el per&iacute;odo y r&iacute;o(s) ingresados',
				    buttons: {
				    	Aceptar: { 
					        }
					    }
			  ...
			  somePlot = $.plot( $("#wrap_chart_plot_02 #chart_plot_02"), 
						dataset,
						{
						  	series: {
							        lines: {
        ...
        if (item) {
            if ((previousLabel != item.series.label) || (previousPoint != item.dataIndex)) {
                ...
                if (item.seriesIndex >= 0) { // Tomara todos
                    ...
                    item.pageY,
                    color,
                    "<strong>" + item.series.label + "</strong><br>DIA " + dayNumber + " : <strong>" + y + "</strong>(m3/s)");
                }
