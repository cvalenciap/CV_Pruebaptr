...
function cargarComponentes() {
	...
	btnBuscarArbol.click(function(event){
		if(validarDatosEntradaPrev()){
			buscarReporteGestionPrev();
		}
	});
	
	formatearInputReporte();
}
...
//	$.redirect("./reporteRegistroLaboratorioPTAR", {"fechaInicio" : dtpInicio.val(), "fechaFin" : dtpFin.val(), "idPtarxSector" : cboPtarxSector.val(), "descripcionPtar" : extraerTextoCombo("cboPtarxSector"), "descripcionSector" : txtSector.val(), "idRegistroLaboratorio" : txtIdRegistroLaboratorio.val(), "arrayParametros" : JSON.stringify(cbParametros.val())});
	$.redirect("./reporteRegistroLaboratorioPTAR", {"idRegistroLaboratorio" : txtIdRegistroLaboratorio.val(), "arrayParametros" : JSON.stringify(cbParametros.val()), "listaTree" : JSON.stringify(listaTreeSelect)});
}
...
function limpiarParametrosBusqueda(){
	...
	$('#divTreeSection').hide();
	listaTreeSelect = [];
	txtIdRegistroLaboratorio.removeAttr("disabled");
	mostrarMensaje();
	formatearInputReporte();
}
...
function buscarReporteGestionPrev(){
	$.ajax({
        url : "./buscarReporteGestionPrev",
        type : "POST",
        dataType : "json",
        async:true,
        data : {
    	...
    	if(respuestaBean.estadoRespuesta ==  ConstanteServices.OK ){	      	
        	...
        	if(listaResultado != null && listaResultado.length > 0 ) {
        		...
        		$("#divTreeSection").show();
        		$("#nodeTittle").text("Nodos Asociados a " + registroLab.descripcionPtar); 
        		construirArbol(listaResultado);
        		txtIdRegistroLaboratorio.attr("disabled", "disabled");
        	}else{
...
function formatearInputReporte(){
	ponerClassConjunto("inputNumerico","idRegistroLaboratorio",8,0,0);
}